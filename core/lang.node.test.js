...
test_block('complex_parse', function() {
    const xs = [
        ...
        '^(#(化滅 (甲) (甲 甲)) (#(化滅 (甲) (甲 甲))))',
        //[性能問題] '%(#(符名 太始初核 (一類何物 化滅 (是非 (一類何物 連)))) (^(#(化滅 (甲) (甲 甲)) (#(化滅 (甲) (甲 甲))))))',
        //[性能問題] '$(#(映表 ()) (#(符名 太始初核 (式形 (太始初核 化滅))) #(符名 太始初核 (一類何物 化滅 解算)) (#(符名 太始初核 (式形 (太始初核 式形))) #(符名 太始初核 (一類何物 式形 引用)) #(映表 ())) ()))',
    ]
    for (const x of xs) {
