    ...
    function complex_parse(x) {
        ...
        function readsysname_no_pack() {
                ...
                case ':':
                    {
                        ...
                        return new_list(a_sym, x_8, sth_sym);
                    }
                case '+':
                    {
                        var x_9 = readsysname_no_pack_inner_must();
                        return new_list(sys_sym, x_9);
                    }
                case '[':
                    {
            ...
            function readsysname_no_pack_inner_must(strict) {
                ...
                var fs = strict ? [list, symbol, readsysname_no_pack_bracket, data, readerror, readeval, readfuncapply, readformbuiltin, readapply] : [list, readsysname_no_pack, data, readerror, readeval, readfuncapply, readformbuiltin, readapply];
                for (var i = 0; i < fs.length; i++) {
                    var x_12 = fs[i]();
                    if (x_12 !== false) {
                        return x_12;
    ...
    function complex_print(val) {
        ...
        switch (type_of(x)) {
            ...
            case data_t:
                ...
                if (maybe_xs !== false && maybe_xs.length === 2 && jsbool_no_force_equal_p(name_1, name_sym) && jsbool_no_force_equal_p(maybe_xs[0], sys_sym)) {
                    // make_sys_sym_f(maybe_xs[1])
                    return print_sys_name(maybe_xs[1], 'top');
                }
                return "#" + complex_print(new_cons(name_1, list));
            case error_t:
