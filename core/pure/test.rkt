  ...
  `(begin
     ...
     ,@(map
        ...
        '(("$(#(映表 ((h g))) h)" "g")
          ...
          ("$(#(映表 ((H P) (B G))) (#(符名 太始初核 (式形 (太始初核 化滅))) #(符名 太始初核 (一類何物 (化滅 省略一物 誤) 特定其物)) B H))" "!(G . P)")
          ("$(#(映表 ()) N)" "!(太始初核 &+化滅 (解算:化滅 (#(映表 ()) N)))")
          ("^(#(化滅 (X) X) (()))" "()")
          ("^(#(化滅 X X) (A B))" "(A B)")
          ("$(#(映表 ()) (#(符名 太始初核 (式形 (太始初核 化滅))) #(符名 太始初核 (一類何物 (化滅 (連) 省略一物) 首)) (#(符名 太始初核 (式形 (太始初核 式形))) #(符名 太始初核 (一類何物 式形 引用)) (8 1 9))))" "8")
